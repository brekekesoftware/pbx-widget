...
export type Pbx = PbxPal & {
  ...
  call_pal<K extends keyof PbxPal>(
    k: K,
    ...p: Parameters<PbxPal[K]>[0] extends undefined ? [] : [Parameters<PbxPal[K]>[0]]
...
export type PbxPal = {
  ...
  getPhonebooks(p: {}, resolve: (res: PbxBook[]) => void, reject: ErrorHandler): void;
  deleteContact(
    p: PbxDeleteContactParam,
    resolve: (res: PbxDeleteContactResponse) => void,
    reject: ErrorHandler,
  ): void;
  getContact(p: PbxGetContactParam, resolve: (res: PbxContact) => void, reject: ErrorHandler): void;
  setContact(p: PbxContact, resolve: (res: PbxContact) => void, reject: ErrorHandler): void;
...
export type Sip = {
  ...
  setWithVideo(sessionId: string, withVideo?: boolean, videoOptions?: VideoOptions): void;
...
export type UcChatClient = {
  ...
  changeStatus(status: string, dislay: string, resolve: () => void, reject: ErrorHandler);
  ...
  receiveUnreadText(resolve: (res: UcReceieveUnreadText) => void, reject: ErrorHandler);
  ...
  acceptFileWithXhr(file: unknown, xhr: XMLHttpRequest, reject: ErrorHandler): void;
